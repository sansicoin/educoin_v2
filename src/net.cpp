...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "EducoinV " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"node.edu-coin.org", "5.9.165.252"},
    {"node1.edu-coin.org", "159.203.123.176"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. EducoinV is probably already running."), addrBind.ToString().c_str());
        else
